...
fun DrawScope.drawCompassGauge(
    gaugeColor: Color,
    gaugeWidth: Float = com.sheltek.kmpweather.compose.ui.utils.GAUGE_WIDTH
) {
...
fun DrawScope.drawNorthArrow() {
    val radius = size.width / 2
    val windDirectionRadians = com.sheltek.kmpweather.compose.ui.utils.RADIANS_OFFSET * (PI / 180)
...
fun DrawScope.drawCardinalLabels(
    ...
    val radius = size.width / 2
    val positions = mapOf(
        com.sheltek.kmpweather.compose.ui.utils.NORTH to Offset(center.x, center.y - radius + 32),
        com.sheltek.kmpweather.compose.ui.utils.EAST to Offset(center.x + radius - 32, center.y),
        com.sheltek.kmpweather.compose.ui.utils.SOUTH to Offset(center.x, center.y + radius - 36),
        com.sheltek.kmpweather.compose.ui.utils.WEST to Offset(center.x - radius + 36, center.y)
    )
...
fun DrawScope.drawCompassNeedle(
    ...
    val windDirectionRadians = (windDirection.degrees + com.sheltek.kmpweather.compose.ui.utils.RADIANS_OFFSET) * (PI / 180)
    ...
    val needlePointEnd = center + Offset(
        x = (radius - com.sheltek.kmpweather.compose.ui.utils.NEEDLE_LINE_LENGTH) * cos(windDirectionRadians).toFloat(),
        y = (radius - com.sheltek.kmpweather.compose.ui.utils.NEEDLE_LINE_LENGTH) * sin(windDirectionRadians).toFloat()
    )
...
fun Path.drawNeedleArrowHead(
    ...
    arrowLength: Float = com.sheltek.kmpweather.compose.ui.utils.ARROW_LENGTH,
    arrowWidth: Float = com.sheltek.kmpweather.compose.ui.utils.ARROW_WIDTH
): Path = Path().apply {
