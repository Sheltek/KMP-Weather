package com.br.kmmdemo.compose.ui.utils
...
import com.br.kmmdemo.compose.ui.weatherDetails.wind.WindDirection
...
fun DrawScope.drawCompassGauge(
    gaugeColor: Color,
    gaugeWidth: Float = com.br.kmmdemo.compose.ui.utils.GAUGE_WIDTH
) {
...
fun DrawScope.drawNorthArrow() {
    val radius = size.width / 2
    val windDirectionRadians = com.br.kmmdemo.compose.ui.utils.RADIANS_OFFSET * (PI / 180)
...
fun DrawScope.drawCardinalLabels(
    ...
    val radius = size.width / 2
    val positions = mapOf(
        com.br.kmmdemo.compose.ui.utils.NORTH to Offset(center.x, center.y - radius + 32),
        com.br.kmmdemo.compose.ui.utils.EAST to Offset(center.x + radius - 32, center.y),
        com.br.kmmdemo.compose.ui.utils.SOUTH to Offset(center.x, center.y + radius - 36),
        com.br.kmmdemo.compose.ui.utils.WEST to Offset(center.x - radius + 36, center.y)
    )
...
fun DrawScope.drawCompassNeedle(
    ...
    val windDirectionRadians = (windDirection.degrees + com.br.kmmdemo.compose.ui.utils.RADIANS_OFFSET) * (PI / 180)
    ...
    val needlePointEnd = center + Offset(
        x = (radius - com.br.kmmdemo.compose.ui.utils.NEEDLE_LINE_LENGTH) * cos(windDirectionRadians).toFloat(),
        y = (radius - com.br.kmmdemo.compose.ui.utils.NEEDLE_LINE_LENGTH) * sin(windDirectionRadians).toFloat()
    )
...
fun Path.drawNeedleArrowHead(
    ...
    arrowLength: Float = com.br.kmmdemo.compose.ui.utils.ARROW_LENGTH,
    arrowWidth: Float = com.br.kmmdemo.compose.ui.utils.ARROW_WIDTH
): Path = Path().apply {
